...
// decodeAlpha reads an 8 bit-per-pixel BMP image from r.

func decodeAlpha(r io.Reader, c image.Config, topDown bool) (image.Image, error) {
	...
	for y := y0; y != y1; y += yDelta {
		if c.Width >= 8 {
			pp := make([]byte, c.Width/8)
			if _, err := io.ReadFull(r, pp); err != nil {
				return nil, err
			}
			for i := 0; i < c.Width; i++ {
				p := rgba.Pix[y*rgba.Stride+i*4 : y*rgba.Stride+(i+1)*4]
				b := pp[i/8]
				b ^= 0xff
				rol := i % 8
				b = b << rol
...
func Decode(r io.Reader) (image.Image, error) {
	...
	switch bpp {
	case 1:
		return decodeAlpha(r, c, topDown)
	case 8:
...
func decodeConfig(r io.Reader) (config image.Config, bitsPerPixel int, topDown bool, err error) {
	...
	case 1:
		if offset > fileHeaderLen+infoLen {
			if _, err := io.ReadFull(r, b[fileHeaderLen+infoLen:offset]); err != nil {
				return image.Config{}, 0, false, err
			}
		}
		return image.Config{ColorModel: color.RGBAModel, Width: width, Height: height}, 1, topDown, nil
	case 8:
